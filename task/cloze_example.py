...
class Config:
    ...
    # 配置路径
    train_data_path = "/home/th/papercode/idiom_cloze/data/train_data.txt"  # 训练集数据的路径，建议绝对路径
    dev_data_path = "/home/th/papercode/idiom_cloze/data/dev_data.txt"  # 验证集数据的路径，建议绝对路径
    test_data_path = "/home/th/papercode/idiom_cloze/data/test_data.txt"  # 测试集数据的路径，建议绝对路径

    # transformer结构(Bert, Albert, Roberta等)的预训练模型的配置, 路径也建议是绝对路径
    bert_model_path = "/home/th/papercode/idiom_cloze/pretrained_models/chinese_wwm_pytorch/pytorch_model.bin"  # 预训练模型路径， 例如bert预训练模型
    model_config_path = "/home/th/papercode/idiom_cloze/pretrained_models/chinese_wwm_pytorch/config.json"  # 预训练模型的config文件路径， 一般是json文件
    vocab_path = "/home/th/papercode/idiom_cloze/pretrained_models/chinese_wwm_pytorch/vocab.txt"  # vocab文件路径，可以是预训练模型的vocab.txt文件
    ...
    # 新增文件路径
    idiom_list_path = "/home/th/papercode/idiom_cloze/data/idiom_list_process.txt"
...
if __name__ == '__main__':
    ...
    if config.do_train:
        ...
        if config.do_eval:
            valid_dataset = task.read_data(config.dev_data_path, mode=Task_Mode.Eval)
            task.train(dataset, valid_dataset=valid_dataset)
        else:
            task.train(dataset)
    if config.do_eval:
        dataset = task.read_data(config.dev_data_path, mode=Task_Mode.Eval)
